...
- **Thread-Sicherheit:** `g_cRef` als `volatile LONG` mit `InterlockedIncrement/Decrement`; `BasketStore` via Named Mutex (siehe Korb-Speicher); zweite Datei-Operation im gleichen Prozess durch `g_fileOpInFlight`-Flag (`volatile LONG`, atomares `InterlockedCompareExchange`-Acquire in `LaunchFileOp`, Release im Thread-Exit — siehe Sektion „File-Op-In-Flight Guard") ausgeschlossen — UI-Thread, Background-FileOp-Thread und parallele Explorer-Prozesse koennen sich nicht mehr in die Quere kommen
...
### File-Op-In-Flight Guard

Zwei parallel laufende `IFileOperation`-Threads im gleichen Explorer-Prozess wuerden auf denselben Quelldateien racen und `operations.log` ueberschreiben (`CREATE_ALWAYS` + `dwShareMode=0` → entweder Clobber oder still verschluckter `CreateFileW`-Fehler, der den Abort-Dialog verschwinden laesst). Verhindert durch process-local `volatile LONG g_fileOpInFlight` (`FileOps.cpp`):

- **Atomares Acquire:** `LaunchFileOp` macht `InterlockedCompareExchange(&g_fileOpInFlight, 1, 0)` und returnt sofort, falls schon belegt — race-tighte Defense, falls zwei Rechtsklicks am Early-Check vorbei rutschen
- **Atomares Release:** `FileOpThreadProc` setzt den Slot via `InterlockedExchange` zurueck — *vor* `InterlockedDecrement(&g_cRef)`, damit ein Follow-Up-Op aus einem evtl. innerhalb von `ExecuteFileOpCOM` geschlossenen Abort-Dialog nicht faelschlich als "busy" abgewiesen wird. Bei `_beginthreadex`-Fehler wird der Slot ebenfalls freigegeben
- **Public API:** `FileOps::IsBusy()` returnt nur den Flag-Status (kein Acquire). `CShellExt::HandleFileOp` checkt das *vor* `BrowseForFolder`, damit der User keinen Picker fuellt, dessen Ergebnis verworfen wird, und zeigt eine lokalisierte `MessageBoxW` (`BusyTitle`/`BusyMsg`) statt den zweiten Op stillschweigend zu verschlucken
- **Scope:** Process-local. Bei Explorer mit Option "Open folder windows in a separate process" hat jeder Prozess seinen eigenen Slot — akzeptierter Trade-off (cross-process waere ein Named Mutex aehnlich `BasketLock`, aber Explorer-Default ist Single-Process)

Der Abort-Dialog (`ShowAbortDialog` in `ExecuteFileOpCOM`) blockiert den Slot solange er offen ist — *gewollt*, damit der User den Incident erst sieht/wegklickt, bevor er die naechste Op startet.
...
- **Lazy Loading (wichtig, frueher rekursiv eager → UI-Hang/Crash bei tiefen/zyklischen Baeumen):** `InsertTreeNode(hParent, name, fullPath, icon, isDirectory, isReparsePoint)` legt einen Knoten an und entscheidet anhand `isDirectory && !isReparsePoint`, ob ein Expand-Pfeil (`cChildren=1`) gezeigt und der volle Pfad als Heap-`std::wstring*` in `lParam` gespeichert wird (sonst `cChildren=0` und `lParam=0`). `PopulateChildren()` laedt **genau eine Ebene** (`FindFirstFileW`/`FindNextFileW`; Verzeichnisse zuerst, dann Dateien, stabile Sortierung; Icons via `GetSysIconIndex()`). `PopulateTreeFromPath()` fuegt Wurzel ein, ruft `PopulateChildren` fuer die erste Ebene, expandiert die Wurzel. Tiefere Ebenen werden beim ersten Klick auf den Expand-Pfeil im `TVN_ITEMEXPANDINGW`-Handler nachgeladen — der prueft via `TreeView_GetChild`, ob der Knoten schon Kinder hat (skip wenn ja), und setzt `cChildren=0` zurueck, falls die Ebene leer war (Pfeil verschwindet). **Wichtig:** `TVIS_EXPANDEDONCE` waere als Guard falsch, weil der Flag vom System erst *nach* dem ersten Expand gesetzt wird — der von `PopulateTreeFromPath` ausgeloeste Initial-Expand der eager-populierten Wurzel wuerde sonst die Doppel-Population der ersten Ebene triggern. Flicker-Schutz: `WM_SETREDRAW` FALSE/TRUE um den Populate-Block
